...
#define __REMOVE_OLD_MFRC255_FUNCTIONS__

#include "main.h"

extern SPI_HandleTypeDef SpiHandle;
#define MFRC522_SPI								&SpiHandle
#define MFRC522_GPIO_PORT						GPIOE
#define MFRC522_CPI_SS_PIN						GPIO_PIN_3
//#define MFRC522_CPI_SDA_PIN						GPIO_PIN_14
//#define MFRC522_CPI_IRQ_PIN						GPIO_PIN_15

//#define MFRC522_CPI_SCK_PIN						GPIO_PIN_2
//#define MFRC522_CPI_MISO_PIN					GPIO_PIN_5
//#define MFRC522_CPI_MOSI_PIN					GPIO_PIN_6


#define MFRC522_CS_LOW							HAL_GPIO_WritePin(MFRC522_GPIO_PORT, MFRC522_CPI_SS_PIN, GPIO_PIN_RESET)
#define MFRC522_CS_HIGH							HAL_GPIO_WritePin(MFRC522_GPIO_PORT, MFRC522_CPI_SS_PIN, GPIO_PIN_SET)
...
typedef struct {
	...
	uint8_t		uids[10];
	uint8_t		sak[3];			// The SAK (Select acknowledge) uint8_t returned from the PICC after successful selection.
	int8_t		index;			// index of the UID in the known UIDs table
} UID;

// A struct named MIFARE_Key used for passing a MIFARE Crypto1 key
typedef struct {
	uint8_t		keyA[MF_KEY_SIZE];//keyuint8_t[MF_KEY_SIZE] is an array with six uint8_ts; keys A and B are each 6 uint8_ts long (see defintion of MF_KEY_SIZE above)
	uint8_t		keyB[MF_KEY_SIZE];//keyuint8_t[MF_KEY_SIZE] is an array with six uint8_ts; keys A and B are each 6 uint8_ts long (see defintion of MF_KEY_SIZE above)
} MIFARE_Key;
...
/////////////////////////////////////////////////////////////////////////////////////
void PCD_WriteRegister(uint8_t reg, uint8_t value);//Writes a uint8_t to the specified register in the MFRC522 chip.
...
StatusCode_t PCD_CalculateCRC(uint8_t *data, uint8_t length, uint8_t *result);
...
/////////////////////////////////////////////////////////////////////////////////////
StatusCode_t PCD_TransceiveData(uint8_t *sendData, uint8_t sendLen, uint8_t *backData, uint8_t *backLen, uint8_t *validBits, uint8_t rxAlign, bool checkCRC);
StatusCode_t PCD_CommunicateWithPICC(uint8_t command, uint8_t waitIRq, uint8_t *sendData, uint8_t sendLen, uint8_t *backData, uint8_t *backLen, uint8_t *validBits, uint8_t rxAlign, bool checkCRC);

StatusCode_t PICC_RequestA(uint8_t *bufferATQA, uint8_t *bufferSize);
StatusCode_t PICC_WakeupA(uint8_t *bufferATQA, uint8_t *bufferSize);
StatusCode_t PICC_REQA_or_WUPA(	uint8_t command, uint8_t *bufferATQA, uint8_t *bufferSize);
//uint8_t PICC_Select(UID *uid, uint8_t validBits);
StatusCode_t PICC_AnticollisionLoop(uint8_t CollisionLevel, UID* puid, uint8_t* anticollision_frame, uint8_t acframe_size, uint8_t* SAK_frame);
...
/////////////////////////////////////////////////////////////////////////////////////
StatusCode_t PCD_Authenticate(uint8_t command, uint8_t blockAddr, MIFARE_Key *key, UID *uid);//this method is used to authenticate a certain block for writing or reading
//command: See enumerations above -> PICC_CMD_MF_AUTH_KEY_A	= 0x60 (=1100000),		// this command performs authentication with Key A
//MIFARE_Key *key is a pointer to the MIFARE_Key struct defined above, this struct needs to be defined for each block. New cards have all A/B= FF FF FF FF FF FF
//Uid *uid is a pointer to the UID struct that contains the user ID of the card.
void PCD_StopCrypto1();
//these two methods read and write entire blocks. It seems there is no method to just access one uint8_t.
StatusCode_t MIFARE_Read(uint8_t blockAddr, uint8_t *buffer, uint8_t *bufferSize);//MIFARE_Read(block number (0-15), uint8_t array containing 16 values, number of uint8_ts in block (=16))
StatusCode_t MIFARE_Write(uint8_t blockAddr, uint8_t *buffer, uint8_t bufferSize);//MIFARE_Write(block number (0-15), uint8_t array containing 16 values, number of uint8_ts in block (=16))
StatusCode_t MIFARE_Decrement(uint8_t blockAddr, long delta);
StatusCode_t MIFARE_Increment(uint8_t blockAddr, long delta);
StatusCode_t MIFARE_Restore(uint8_t blockAddr);
StatusCode_t MIFARE_Transfer(uint8_t blockAddr);
StatusCode_t MIFARE_Ultralight_Write(uint8_t page, uint8_t *buffer, uint8_t bufferSize);


/////////////////////////////////////////////////////////////////////////////////////
// Support functions
/////////////////////////////////////////////////////////////////////////////////////
StatusCode_t PCD_MIFARE_Transceive(	uint8_t *sendData, uint8_t sendLen, bool acceptTimeout);
...
StatusCode_t PICC_WriteTrailerBlock(int blockNumber,
		struct_block_access_bits* B0,
		struct_block_access_bits* B1,
		struct_block_access_bits* B2,
		struct_block_access_bits* TB,
